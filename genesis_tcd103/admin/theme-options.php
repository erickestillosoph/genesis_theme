...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-genesis' ); ?></h2> 
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-genesis' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-genesis'); ?></p></div>
  </div>
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-genesis' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-genesis' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-genesis' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-genesis' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-genesis' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-genesis' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-genesis' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-genesis' ) . '</strong></p></div>';
	}
}
...
/**
 * genesis オプションTools デフォルト画像設定フィルター
 */
function genesis_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			...
			'theme_option_keys' => array("index_slider_image")
		),
		array(
			'media_filename' => 'image_1450x500.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x500.gif',
			'theme_option_keys' => array("footer_banner_image1","footer_banner_image2")
		),
		array(
			'media_filename' => 'image_623x450.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_623x450.gif',
			'theme_option_keys' => array("['contents_builder'][0]['item_image1']","['contents_builder'][0]['item_image2']")
		)
	);
}
...
/**
 * genesis オプションTools サンプルカテゴリー
 */
function genesis_theme_options_tools_set_sample_categories( $dp_options ) {
	...
	for ( $i = 1; $i <= 3; $i++ ) {
		$sample_categories[] = array(
			'name' => __( 'Category', 'tcd-genesis' ) . $i,
			'slug' => 'category'.$i,
			'taxonomy' => 'category',
		);
	}
  ...
  // サービスカテゴリー
	$service_cat_image = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1450x600.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x600.gif'
	), true );
	$service_cat_image2 = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_824x1160.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_824x1160.gif'
	), true );
	for ( $i = 1; $i <= 3; $i++ ) {
		$sample_categories[] = array(
			'name' => __( 'Category', 'tcd-genesis' ) . $i,
			'slug' => 'service_category'.$i,
			...
			'metas' => array(
				...
				'desc2_mobile' => '',
				'desc3_mobile' => '',
				'image1' => $service_cat_image,
				'post_list_type' => 'type1',
				'index_image' => $service_cat_image2,
				'index_image_mobile' => '',
...
/**
 * genesis オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function genesis_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_840x560.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_840x560.gif'
	), true );

	// サービスのアイキャッチ画像
	$service_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1450x600.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x600.gif'
	), true );
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-genesis' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-genesis' ),
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-genesis' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-genesis' ),
			...
			'thumbnail' => $media_id,
			'taxonomies' => array(
				'news_category' => array('news_category1','news_category2','news_category3')
			),
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample service', 'tcd-genesis' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-genesis' ),
			'post_name' => 'sample-service' . $i,
			'post_status' => 'publish',
			'post_type' => 'service',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $service_media_id,
			'taxonomies' => array(
				'service_category' => array('service_category1','service_category2','service_category3')
			),
			'metas' => array(
				'service_link_list_headline' => __( 'Headline', 'tcd-genesis' ),
				'service_link_list_sub_title' => __( 'Subtitle', 'tcd-genesis' ),
				'service_link_list' => '',
			)
		);
	}
...
/**
 * genesis オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function genesis_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( ! empty( $menu_locations['drawer-menu'] ) && $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->term_id == $menu_locations['drawer-menu'] )
				return $dp_options;
		}
	}
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-genesis' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-genesis' ) );
	...
	$menu_locations['drawer-menu'] = (int) $menu_id;
...
/**
 * genesis オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function genesis_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
        'title1' => __('Recent post', 'tcd-genesis'),
        'title2' => __('Recent post', 'tcd-genesis'),
        'title3' => __('Recent post', 'tcd-genesis'),
        'post_num1' => '3',
        'post_num2' => '3',
        'post_num3' => '3',
        'content_type1' => 'post',
 ...
 * genesis テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'genesis_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'genesis_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'genesis_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'genesis_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'genesis_theme_options_tools_set_sample_widgets', 10, 1 );
