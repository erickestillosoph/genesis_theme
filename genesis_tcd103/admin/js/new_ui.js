jQuery(function ($) {
  ...
  themeOptionsForm.on("change", ".qt-btn-preview-wrapper :input", function () {
    ...
    var color2 = $cl.find('[name*="_color2"]').val();
    var hoverColor2 = $cl.find('[name*="_color2_hover"]').val();

    // 通常時のカラー
    var obj = {
      type1: [color, color2, color2],
      type2: [color, color2, "transparent"],
      type3: [color, color2, "transparent"],
    };
    Object.keys(obj).forEach((key) => {
      var target = $cl.find(".qt_button." + key);
      target.css({
        color: obj[key][0],
        borderColor: obj[key][1],
        backgroundColor: obj[key][2],
      });
      if (key == "type3") {
        target.find(".background").css("backgroundColor", color2);
      }
    ...
    // ホバー時の配色
    obj = {
      type1: [hoverColor, hoverColor2, hoverColor2],
      type2: [hoverColor, hoverColor2, hoverColor2],
      type3: [hoverColor, hoverColor2, "transparent"],
    };
    Object.keys(obj).forEach((key) => {
      var target = $cl.find(".qt_button." + key);
  ...
  themeOptionsForm.on(
    "change",
    ".qt-frame-preview-wrapper :input",
    function () {
      var $cl = $(this).closest(".qt-frame-preview-wrapper");
      var $preview = $cl
        .find(".qt-frame-preview")
        .children()
        .removeAttr("style");
  ...
  themeOptionsForm.on("change", ".qt-ul-preview-wrapper :input", function () {
    ...
    $preview.css(
      ...
      "linear-gradient(to right, transparent 50%, " +
        $cl.find('[name*="_border_color"]').val() +
        " 50%)"
    );
    $preview.css(
      "fontWeight",
      $cl.find('[name*="_font_weight"]:checked').val()
    );
    $cl
      .find(".qt-ul-preview > div")
      .removeClass()
      .addClass($cl.find('[name*="_use_animation"]:checked').val());
  });
    ...
    Object.keys(obj).forEach(function (key) {
      ...
      if (key == "backgroundColor") {
        if ($cl.find(".qt_speech_bubble").hasClass("right")) {
          $preview
            .find(".after")
            .css("borderLeftColor", $cl.find(obj[key]).val());
        } else {
          $preview
            .find(".after")
            .css("borderRightColor", $cl.find(obj[key]).val());
        }
      }
    });
  });
  ...
  themeOptionsForm.on(
    "change",
    ".single-cta-preview-wrapper :input",
    function () {
      ...
      $preview.css(
        "fontSize",
        $cl.find('[name*="_catch_font_size"]').val() + "px"
      );
  ...
  themeOptionsForm.on(
    "change",
    ".mini-cta-preview-wrapper :input",
    function () {
      ...
      // キャッチフレーズ
      var title = $cl
        .find('textarea[name="dp_options[mini_cta_catch]"]')
        .val()
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        ...
        .replace(/\r?\n/g, "<br />");
      var title_color = $cl
        .find('input[name="dp_options[mini_cta_catch_font_color]"]')
        .val();
      if (title === "") {
        $preview
          .find(".mini_cta_catch")
          .removeAttr("style")
          .css({ margin: 0, color: title_color })
          .find(".catch")
          .html(title);
      } else {
        $preview
      ...
      // 説明文
      var desc = $cl
        .find('textarea[name="dp_options[mini_cta_desc]"]')
        .val()
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
      ...
      // ボタン
      var button_label = $cl
        .find('input[name="dp_options[mini_cta_button_label]"]')
        .val()
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        ...
        .replace(/\r?\n/g, "<br />");
      var button_color = $cl
        .find('input[name="dp_options[mini_cta_button_bg_color]"]')
        .val();
      $preview
        .find(".mini_cta_button")
        .removeAttr("style")
        .css("backgroundColor", button_color)
        .html(button_label);
    }
  );
  ...
  themeOptionsForm.on(
    "change",
    ".modal-cta-preview-wrapper :input",
    function () {
      ...
      // キャッチフレーズ
      var title = $cl
        .find('textarea[name="dp_options[modal_cta_catch]"]')
        .val()
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        ...
        .replace(/\r?\n/g, "<br />");
      $preview
        .find(".modal_cta_catch")
        .html(title)
        .css(
          "fontSize",
          $cl
            .find('input[name="dp_options[modal_cta_catch_font_size]"]')
            .val() + "px"
        );

      // 背景色と透明度
      $preview
        .find(".modal_cta_overlay")
        .removeAttr("style")
        .css({
          backgroundColor: $cl
            .find('input[name="dp_options[modal_cta_overlay_color]"]')
            .val(),
          opacity: $cl
            .find('input[name="dp_options[modal_cta_overlay_opacity]"]')
            .val(),
        });
    }
  );
  ...
  $(".full_screen_button").on("click", function () {
    ...
    $(document).on(
      "click",
      ".modal_cta.is_open .modal_cta_close, .modal_cta.is_open .full_screen_overlay",
      function () {
  ...
  $(document).on("click", ".tcd_standard_tab_label", function (event) {
    ...
    $(this)
      .addClass("is_active")
      .siblings(".tcd_standard_tab_label")
      .removeClass("is_active");
    $(this)
      .closest(".tcd_standard_tab_area")
      .next(".tcd_standard_tab_contents")
      .find(".tcd_standard_tab_content")
      .removeClass("is_active");
  ...
  themeOptionsForm.on(
    "change",
    ".design_headline-preview-wrapper :input",
    function () {
  ...
  $(document).on("click", ".js-color-preset-item", function () {
    ...
    $(".js-color-preset-target--link").wpColorPicker("color", mainColor);
