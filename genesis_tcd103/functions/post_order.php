...
function po_admin_print_scripts() {
	...
	if ('page' === $post_type || 'service' === $post_type) {
...
/**
 * Change post order of custom post type 'gallery' and 'company'
 */
function change_post_order() {
	...
	if ( 'page' !== $post_type && 'service' !== $post_type) return;
...
function po_pre_get_posts( $query ) {
  ...
  // Modify post order of gallery and company
  //if ( is_admin() || ! $query->is_main_query() ) return;
  if ( is_admin() ) return;
  if ( (is_post_type_archive('service') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
  if ( (is_tax('service_category') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
}
...
function po_get_previous_post_where( $where, $in_same_term, $excluded_terms, $taxonomy, $post ) {
	if ( in_array( $post->post_type, ['service'] ) ) {
