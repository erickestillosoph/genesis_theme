...
function tcd_image_editor_enqueue_script() {
	...
	wp_localize_script(
		...
		'TCD_IMAGE_EDITOR',
		array(
			'ajax_error_message' => __( 'Error was occurred. Please retry again.', 'tcd-genesis' ),
			'saving'             => __( 'Saving', 'tcd-genesis' ),
			'restoring'          => __( 'Restoring', 'tcd-genesis' ),
		)
	);
}
...
<div id="tcd-image-editor" class="tcd-image-editor">
	<div tabindex="0" class="media-modal wp-core-ui">
		<button type="button" class="media-modal-close"><span class="media-modal-icon"><span class="screen-reader-text"><?php _e( 'Close media panel', 'tcd-genesis' ); ?></span></span></button>
		<div class="media-modal-content">
			<div class="media-frame wp-core-ui hide-menu hide-router">
				<div class="media-frame-menu"></div>
				<div class="media-frame-title"><h1><?php _e( 'Image Edit', 'tcd-genesis' ); ?></h1></div>
				...
				<div class="media-frame-toolbar">
					<div class="media-toolbar">
						...
						<div class="media-toolbar-primary">
							<button type="button" class="button media-button button-primary button-large media-button-save" disabled="disabled"><?php _e( 'Save Image', 'tcd-genesis' ); ?></button>
...
function tcd_image_editor_ajax() {
	...
	if ( ! isset( $_POST['post_id'], $_POST['media_id'], $_POST['_ajax_nonce'], $_POST['do'] ) ) {
		$json['message'] = __( 'Invalid request.', 'tcd-genesis' );
	} else {
		...
		if ( empty( $post->post_type ) ) {
			$json['message'] = __( 'Invalid post id.', 'tcd-genesis' );
		} elseif ( ! current_user_can( 'edit_post', $post_id ) ) {
			$json['message'] = __( 'You do not have permission to edit this post.', 'tcd-genesis' );
		} elseif ( ! $image ) {
			$json['message'] = __( 'Invalid media id.', 'tcd-genesis' );
		} elseif ( ! current_user_can( 'edit_post', $media_id ) ) {
			$json['message'] = __( 'You do not have permission to edit this image.', 'tcd-genesis' );
		} elseif ( 'open' === $_POST['do'] ) {
			// エディター表示

			if ( ! check_ajax_referer( 'tcd-image-edit-' . $post_id, '_ajax_nonce', false ) ) {
				$json['message'] = __( 'Invalid nonce.', 'tcd-genesis' );
			} else {
				...
				if ( $results && is_array( $results ) ) {
					...
					$json['message'] = __( 'Failed to get image data.', 'tcd-genesis' );
				}
			}
			...
			if ( ! check_ajax_referer( 'tcd-image-edit-save-' . $media_id, '_ajax_nonce', false ) ) {
				...
				if ( true === $results && $image ) {
					...
					$json['message'] = __( 'Failed to save image data.', 'tcd-genesis' );
				}
			}
			...
			if ( ! check_ajax_referer( 'tcd-image-edit-save-' . $media_id, '_ajax_nonce', false ) ) {
				...
				if ( isset( $restore_image->msg ) ) {
					...
					if ( $results && is_array( $results ) ) {
						...
						$json['message']  = __( 'Failed to restore image.', 'tcd-genesis' );
						$json['message'] .= $results;
					} else {
						$json['message']  = __( 'Failed to restore image.', 'tcd-genesis' );
						$json['message'] .= __( 'Failed to get image data.', 'tcd-genesis' );
...
function get_tcd_image_editor_data( $media_id, $post_id ) {
	...
	if ( ! $meta || ! is_array( $meta ) ) {
		return __( 'Image data does not exist. Please re-upload the image.', 'tcd-genesis' );
	}
...
<h2 class="tcd-image-editor__filters-headline"><?php _e( 'Filters', 'tcd-genesis' ); ?></h2>
<table class="tcd-image-editor__filters">
	<tr>
		<th><?php _e( 'Brightness', 'tcd-genesis' ); ?></th>
		<td>
		...
		<th><?php _e( 'Contrast', 'tcd-genesis' ); ?></th>
		<td>
	...
	<tr>
		<th><?php _e( 'Saturation', 'tcd-genesis' ); ?></th>
		<td>
	...
	<tr>
		<th><?php _e( 'Sepia', 'tcd-genesis' ); ?></th>
		<td>
		...
		<th><?php _e( 'Sharpen', 'tcd-genesis' ); ?></th>
		<td>
...
<h2 class="tcd-image-editor__preset-filters-headline"><?php _e( 'Preset filters', 'tcd-genesis' ); ?></h2>
<p>
	<?php
	foreach ( array(
		'clarity'   => __( 'Clarity', 'tcd-genesis' ),
		'nostalgia' => __( 'Nostalgia', 'tcd-genesis' ),
		'lomo'      => __( 'Lomo', 'tcd-genesis' ),
		'pinhole'   => __( 'Pin Hole', 'tcd-genesis' ),
		'oldBoot'   => __( 'Old Boot', 'tcd-genesis' ),
		'hazyDays'  => __( 'Hazy Days', 'tcd-genesis' ),
	) as $key => $label ) :
...
<h2 class="tcd-image-editor__reset-headline"><?php _e( 'Reset', 'tcd-genesis' ); ?></h2>
<p>
	<button type="button" class="button button-secondary button-reset-filters"><?php echo _e( 'Reset Filters', 'tcd-genesis' ); ?></button>
	<button type="button" class="button button-secondary button-reset-to-orig hidden"><?php echo _e( 'Reset to Original Image', 'tcd-genesis' ); ?></button>
</p>
...
function save_tcd_image_editor_data( $media_id ) {
	...
	if ( empty( $_FILES['file-image-edited']['tmp_name'] ) ) {
		return __( 'Empty upload image file.', 'tcd-genesis' );
	}
	...
	if ( ! empty( $wp_handle_upload['error'] ) ) {
		...
		return __( 'Failed to upload edited image file.', 'tcd-genesis' );
	}
