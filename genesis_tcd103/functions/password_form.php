...
<div class="tcd_custom_fields">
	<div class="tcd_cf_content">
		<select name="pw_content">
			<option value=""><?php _e( 'Select a content to encourage member registration.', 'tcd-genesis' ); ?></option>
			<?php for ( $i = 1; $i <= 3; $i++ ) : ?>
			<option value="<?php echo $i; ?>" <?php selected( $i, $post->pw_content ); ?>><?php echo tcd_admin_label('content') . $i . ' : ' . esc_html( $dp_options['pw_name' . $i] ); ?></option>
			<?php endfor; ?>
		</select>
	</div><!-- END .content -->
...
function tcd_password_form() {
	...
	if ( $pw_content ) {
		...
		$output .= '<div class="c-pw__desc">' . apply_filters( 'the_content', $dp_options['pw_editor' . $pw_content] ) . '</div>';
	...
	if ( ! $pw_content ) {
		...
		$output .= '<div class="c-pw__box-inner">';
		$output .= '<input class="c-pw__box-input" name="post_password" id="' . $label . '" type="password" placeholder="'.__( 'Password:', 'tcd-genesis' ).'" size="20"><input class="c-pw__btn c-pw__btn--submit" type="submit" name="Submit" value="' . esc_attr( __( 'Submit', 'tcd-genesis' ) ) . '"></div>';

	} else {

		$output .= '<div class="c-pw__box-inner">';

		$pw_content_label = ( $dp_options['pw_label' . $pw_content] ) ? esc_html( $dp_options['pw_label' . $pw_content] ) : __( 'Password', 'tcd-genesis' );
		$pw_password_button_label = ( $dp_options['pw_password_button_label' . $pw_content] ) ? esc_attr( $dp_options['pw_password_button_label' . $pw_content] ) : __( 'Submit', 'tcd-genesis' );

		if ( $dp_options['pw_label' . $pw_content] ) { 
		}

		$output .= '<input class="c-pw__box-input" name="post_password" id="' . $label . '" type="password" placeholder="'. $pw_content_label .'" size="20"><input class="c-pw__btn c-pw__btn--submit" type="submit" name="Submit" value="' . $pw_password_button_label . '"></div>';
