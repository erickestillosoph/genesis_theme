...
function seo_meta_box() {
  ...
  $post_types = array ( 'post', 'page', 'news', 'service');
  add_meta_box(
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Title tag', 'tcd-genesis' ); ?></h3>
  <input type="text" name="tcd-w_meta_title" value="<?php if(!empty($seo_title)){ echo esc_attr($seo_title); }; ?>" style="width:100%" />
 </div><!-- END .content -->

 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Meta description tag', 'tcd-genesis' ); ?></h3>
  <p><?php printf(__('Recommended number of characters is %s.', 'tcd-genesis'), '180'); ?></p>
...
function seo_title( $title, $sep ) {
	...
	global $post, $page, $paged, $blog_label;
  $options = get_design_plus_option();
	...
	// 各タクソノミーアーカイブ
    	 ...
    	 $title = $term_meta['meta_title'];
			 ...
			 $title = sprintf( __( 'Post list for %s', 'tcd-genesis' ), $term_name );
		 }

	// 検索結果ページ
 	} elseif ( is_search() ) {
    if ( !empty( get_search_query() ) ) {
      $title = sprintf( __( 'Search result for %s', 'tcd-genesis' ), get_search_query() );
    } else {
    ...
    $author_name = sprintf( __( '%s blog list', 'tcd-genesis' ), $curauth->display_name );
	...
	// それ以外
  	...
  	$title .= $site_title;
   	...
   	if ( $paged >= 2 || $page >= 2 ) {
     	$title = "$title $sep " . sprintf( __( 'Page %s', 'tcd-genesis' ), max( $paged, $page ) );
		}
...
function get_seo_description() {
  ...
  $site_description = get_bloginfo( 'description', 'display' );
    ...
    return sprintf( __( 'Archive for %s', 'tcd-genesis' ), get_the_time( __( 'F jS, Y', 'tcd-genesis' ) ) );
 	} elseif ( is_month() ) {
    return sprintf( __( 'Archive for %s', 'tcd-genesis' ), get_the_time( __( 'F, Y', 'tcd-genesis' ) ) );
 	} elseif ( is_year() ) {
    return sprintf( __( 'Archive for %s', 'tcd-genesis' ), get_the_time( __( 'Y', 'tcd-genesis' ) ) );
 	} elseif ( is_author() ) {
    global $wp_query;
    $curauth = $wp_query->get_queried_object();
    return sprintf( __( 'Archive for %s', 'tcd-genesis' ), esc_html( $curauth->display_name ) );
 	} elseif ( is_search() ) {
    return sprintf( __( 'Post list for %s', 'tcd-genesis' ), get_search_query() );
